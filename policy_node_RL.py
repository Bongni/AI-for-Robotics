...
# Author note, I use ### to show where changes were made to integrate CC with lane-keeping
...
from evaluation_for_autonomous_driving import (
...
# Class for detecting when the desired speed has changed. This is for turning off the integrator to reduce overshoot 
# until reaching an acceptable region.
...
class PolicyNode(Node):
    ...
    def __init__(self):
        ...
        self.RLmodel =RLPolicy(PPO.load(r"/home/asc/ai4r-system/ros2_ws/src/ai4r_pkg/scripts/models/RL_baseline.zip"))
        ### Initialise a timer
        ...
        self.prev_lateral_error = 0.0  # lateral error at previous time step for D term
        self.integral_lateral_error = 0.0  # integral of lateral error for I term ###

        self.int_cc_error = 0.0 # integral of error for CC
        self.prev_speed_error = 0.0 # Error at previous time step
        self.current_esc_action = 15.0 ## Is there a point with this if I am not using esc_action at previous time step?
    ...
    def cv_cone_detection_callback(self, msg):
        ...
        if (self.fsm_state == FSM_STATE_PUBLISHING_POLICY_ACTION):
            ...
            # + PID and Gain values
            P_STEERING = 0.2  # P value for Steering Action; SIMULATION=0.0005; DEFAULT=0.25
            I_STEERING = 0.0025  # I value for Steering Action; SIMULATION=0.000002; DEFAULT=0.001
            D_STEERING = 0.025  # D value for Steering Action; SIMULATION=0.000025; DEFAULT=0.0125
            HEADING_GAIN = 0.2  # + Gain for heading error correction; SIMULATION=0.0001; DEFAULT=0.05

            INTEGRAL_ADAPTION = 0.9

            # + Steering Adjustment Values
            STEER_OFFSET = 0  # Offset to adjust for biased steering; DETAULT=0
            ### FIXED_ESC_VALUE = 15.0  #15.0  # Fixed ESC value for steering tests; DEFAULT=33.0

            # + Lookahead Window Definition(in mm)
            X_MIN = 200.0  # Start of lookahead window; SIMULATION=500.0; DETAULT=50.0
            X_MAX = 3_000.0  # End of lookahead window; SIMULATION=8_000.0; DEFAULT=2_000.0
            X_LOOKAHEAD = 800.0  # Lookahead point (where we aim to be in the centre of the road); SIMULATION=1_000.0; DEFAULT=800.0
            ...
            # Your current lists (variable length)
            # x_coords, y_coords, cone_colour
            ...
            x_coords,y_coords,cone_colour= filter_forward_cones(x_coords, y_coords, cone_colour)
            x_coords = list(x_coords)
            y_coords = list(y_coords)
            cone_colour = list(cone_colour)
            num_cones = len(x_coords)
            current_cone_count = num_cones
            MAX_CONES = 48
            
            padding_needed = MAX_CONES - current_cone_count

            # 3. Apply padding
            # Pad X and Y with 0.0
            x_coords.extend([0.0] * padding_needed)
            y_coords.extend([0.0] * padding_needed)

            # Pad Cone Colour with 2 (placeholder for unknown/padding color)
            cone_colour.extend([2] * padding_needed)


            observation = { 'recommended_speed': [5.0],
                           'vx_sensor': [wheel_speed_in_meters_per_second],
                            'cone_detections_x': x_coords,
                            'cone_detections_y': y_coords,
                            'cone_detections_side_of_road': cone_colour,
                            'cone_detections_num_cones': [num_cones]}
            action = self.RLmodel.compute_action(observation, {}, False,False)
            esc_ratio = action[0]
            steering_ratio = action[1]
            esc_action = 15.0*esc_ratio
            steering_action = 100.0*steering_ratio 

            #self.get_logger().info("Speed error is " + str(speed_error))
            self.get_logger().info("Linear speed is " + str(wheel_speed_in_meters_per_second) + "m/s")
           
            self.get_logger().info("ESC action is " + str(esc_action))
            self.get_logger().info("ESC ratio is " + str(esc_ratio))
            self.get_logger().info("Steering action is " + str(steering_action))
            # self.get_logger().info("Heading error is" + str(heading_error))
                    ### esc_action = FIXED_ESC_ACTIONS
    ...
    def fsm_transition_request_callback(self, msg):
        ...
        if requested_state == FSM_STATE_PUBLISHING_POLICY_ACTION:
            self.start_time = self.get_clock().now() # Set start time when publishing policy action
